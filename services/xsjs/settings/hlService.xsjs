...
function handleGet() {
	...
	try{
		...
		var fnSell = conn.loadProcedure('PLANNING_TOOL', 'xsplanningtool.db.procedures::GET_ALL_HIERARCHY_LEVEL');
		  var result = fnSell();
		  var result = result['output_result'];
		  conn.close();
		  handleResponse({"code": $.net.http.OK, "data": {"results": result}}, $.net.http.OK);
		/*var conn = $.db.getConnection();
		var query = 'call "PLANNING_TOOL"."xsplanningtool.db.procedures::GET_ALL_HIERARCHY_LEVEL"(?)';
		...
		while(spResultSet.next()) {
		    spResult.push({
		        "HIERARCHY_LEVEL_ID": spResultSet.getInteger(1),
		        "DESCRIPTION": spResultSet.getString(2)
		    });
		}
